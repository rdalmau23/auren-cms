...
import { Clock, Plus, Trash2, Calendar, Pill, Save, AlertCircle, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {schedules?.map((schedule) => (
            <div key={schedule.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col group">
      ...
      <SlideOver
        ...
        title="Nueva Pauta (Schedule)"
        description="Crea una plantilla de tomas para un fármaco."
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-6 border-t border-gray-100 flex justify-end gap-3">
            ...
            >
              <Save size={16} />
              {createMutation.isPending ? 'Guardando...' : 'Guardar Pauta'}
            </button>
          </div>
        </form>
      </SlideOver>
